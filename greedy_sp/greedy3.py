...
def greedy3_prepare(state):
    ...
    past_pattern = None
    while abs(tracker.state[0]) ** 2 < 0.95:
...
if __name__ == "__main__":
    ...
    state = extract_state_from_file("temp.txt", 5)
    print_state(state)
    gates = greedy3_prepare(state)
    print(sum(int(g.is_cx()) for g in gates))
