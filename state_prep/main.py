...
def to_torch(x):
    return torch.from_numpy(x).cfloat()
...
for state, seq in data.data:
    ...
    for gate in seq:
        ...
        states.append(torch.from_numpy(state).cfloat())
...
class AngleNN(nn.Module):
    def __init__(self):
        ...
        self.l1 = nn.Linear(8, 16, dtype=torch.cfloat)
        self.l2 = nn.Linear(16, 16, dtype=torch.cfloat)
        self.l3 = nn.Linear(16, 16, dtype=torch.cfloat)
        self.l4 = nn.Linear(16, 3, dtype=torch.cfloat)
...
class TargetNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.tanh(self.l2(x))
        x = torch.tanh(self.l3(x))
        x = torch.nn.functional.softmax(self.l4(x).abs(), dim=0)
        return x
...
class ControlNN(nn.Module):
    def __init__(self):
        ...
        self.l2 = nn.Linear(16, 16, dtype=torch.cfloat)
        self.l3 = nn.Linear(16, 16, dtype=torch.cfloat)
        self.l4 = nn.Linear(16, 4, dtype=torch.cfloat)
...
state = states[0].numpy()
for _ in range(20):
    print(state)
    s = torch.from_numpy(state).cfloat()
