...
int main(int argc, char* argv[]) {
  if(argc < 5) {
    std::cout << "Usage: ./isa_cpp [file_output] [file_input] [connectivity graph] [n_qubits] [fidelity]" << std::endl;
    return 0;
  }
  int n_qubits = std::stoi(argv[4]);
  double fidelity = 0.95;
  if(argc > 5) {
    fidelity = std::stod(argv[5]);
  }
  Graph g = Graph::fromFile(std::string(argv[3]));
  ...
  gate_sequence gates = greedy3_prepare(state, n_qubits, fidelity, g);
