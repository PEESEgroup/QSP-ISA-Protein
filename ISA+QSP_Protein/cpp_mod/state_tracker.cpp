...
void StateTracker::control_rotate_merge(int control, int src, int dest) {
    this->unify_phase(src, dest);
    int target = get_target(src, dest);
...
int get_target_pattern(Pattern src, Pattern dest) {
    return get_target(src.bits, dest.bits);
}
...
std::pair<double, double> chunk_angle(const quantum_state& state, 
  Pattern src, Pattern dest) {
    ...
    bool down_rotate = src.get_bit(target) == BitType::ONE;
    quantum_state v0;
    quantum_state v1;
    if(down_rotate) {
...
void StateTracker::control_rotate_merge_chunk(int control, Pattern src, Pattern dest) {
