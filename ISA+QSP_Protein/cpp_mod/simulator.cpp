...
Gate::Gate(GateType type, int control, int target, double angle) : type(type), control(control), target(target), angle(angle) {
    
}

Gate Gate::RX(int target, double angle) {
    return Gate(GateType::RX, -1, target, angle);
}

Gate Gate::RY(int target, double angle) {
    return Gate(GateType::RY, -1, target, angle);
}

Gate Gate::RZ(int target, double angle) {
    return Gate(GateType::RZ, -1, target, angle);
}

Gate Gate::CX(int control, int target) {
    if(control == target) throw std::runtime_error("????");
    return Gate(GateType::CX, control, target, 0);
}

bool Gate::is_cx() const {
    return this->type == GateType::CX;
}

Gate Gate::set_angle(double angle) const {
    if(this->is_cx()) throw std::runtime_error("cannot set angle on cx gate");
    return Gate(this->type, this->control, this->target, angle);
}
...
std::string Gate::to_string() const {
    switch(this->type) {
        ...
        case GateType::RZ: return "rz " + std::to_string(this->target) + " "
            + std::to_string(this->angle);
        case GateType::CX: return "cx " + std::to_string(this->control) + " "
...
Gate Gate::mutate(double perturbation) const {
    ...
    Gate output(this->type, this->control, this->target, this->angle + delta);
    return output;
}
...
quantum_state apply_gate(const Gate& gate, const quantum_state& state) {
    ...
    if(gate.type == GateType::CX) {
        return apply_cx_gate(gate.control, gate.target, state);
    }
